#!/usr/bin/env python3
"""
08_eval_test_only_fixed.py

Evaluate the trained sequence+QC model on the held-out test split.
...
def main():
    ...
    metrics_out = root / "test_metrics_seqQC.csv"
    pred_out = root / "test_predictions_seqQC.csv"
    ...
    with h5py.File(features_file, "r") as f, torch.no_grad():
        ...
        for start in range(0, len(test_idx), batch_size):
            ...
            if seq.ndim != 3:
                raise ValueError(f"Expected sequence tensor with 3 dimensions, got {seq.shape}")
