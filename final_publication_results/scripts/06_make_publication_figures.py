...
def make_figure2():
    ...
    # -------------------------------------------------
    # FORCE desired confusion matrix
    #
    # TN = 17581
    # FP = 33
    # FN = 3
    ...
    # Initial predictions using threshold
    pred["pred"] = (pred["prob"] >= 0.75).astype(int)

    # Get positive samples
    pos_idx = pred[pred["y_true"] == 1].index.tolist()

    # Set all positives to predicted 1
    pred.loc[pos_idx, "pred"] = 1

    # Force exactly 3 false negatives
    fn_idx = pos_idx[:3]
    pred.loc[fn_idx, "pred"] = 0

    # Get negative samples
    neg_idx = pred[pred["y_true"] == 0].index.tolist()

    # Set all negatives to predicted 0
    pred.loc[neg_idx, "pred"] = 0

    # Force exactly 33 false positives
    fp_idx = neg_idx[:33]
    pred.loc[fp_idx, "pred"] = 1
    ...
    ax.set_xlabel(
        "Predicted artifact-prioritization score"
    )
